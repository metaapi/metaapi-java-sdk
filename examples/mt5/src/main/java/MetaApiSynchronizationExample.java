...
public class MetaApiSynchronizationExample {
    ...
    private static final String serverDatFile = getEnvOrDefault("PATH_TO_SERVERS_DAT", "/path/to/your/servers.dat");
        ...
        try {
            ...
            if (!profile.isPresent()) {
                ...
                profile = Optional.of(api.getProvisioningProfileApi().createProvisioningProfile(newDto)
                        .toCompletionStage().toCompletableFuture().get());
                profile.get().uploadFile(serverDatFile)
                        .toCompletionStage().toCompletableFuture().get();
            }
            ...
            connection.subscribeToMarketData("EURUSD").toCompletionStage().toCompletableFuture().join();
